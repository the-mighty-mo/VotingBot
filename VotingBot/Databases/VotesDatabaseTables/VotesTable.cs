...
namespace VotingBot.Databases.VotesDatabaseTables
{
    public class VotesTable : ITable
    {
        ...
        {
            using SqliteCommand cmd = new("CREATE TABLE IF NOT EXISTS Votes (guild_id TEXT PRIMARY KEY, vote_id INTEGER NOT NULL, message TEXT NOT NULL, UNIQUE(guild_id, vote_id));", connection);
        ...
        public async Task<int> GetLastVoteIdAsync(SocketGuild g)
        {
            ...
            using SqliteCommand cmd = new(getVoteId, connection);
            cmd.Parameters.AddWithValue("@guild_id", g.Id);

            SqliteDataReader reader = await cmd.ExecuteReaderAsync();
            if (await reader.ReadAsync())
            {
                _ = int.TryParse(reader["vote_id"].ToString()!, out lastVoteId);
            }
            reader.Close();
